...
let build_lib (lib : Library.t) ~native_archives ~sctx ~expander ~flags ~dir
  ...
  |> Memo.Result.iter ~f:(fun compiler ->
         ...
         let stubs_flags =
           ...
           let make_args ~stub_mode archive =
             let lname =
               "-l"
               ^ Foreign.Archive.(
                   name ~mode:stub_mode archive |> Name.to_string)
             in
         ...
         let ctypes_cclib_flags =
           Ctypes_rules.ctypes_cclib_flags sctx ~expander
...
let cctx (lib : Library.t) ~sctx ~source_modules ~dir ~expander ~scope
  ...
  Compilation_context.create () ~super_context:sctx ~expander ~scope ~obj_dir
    ...
    ~lib_top_module_map ~lib_to_entry_modules_map ~entry_names_closure
