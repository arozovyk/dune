...
module Includes = struct
  ...
  let filter_with_odeps libs deps md lib_top_module_mapm
 ...
 *)
    ...
    then Resolve.Memo.return libs
      ...
      let lib_not_is_filterable lib =
        ...
        implements || virtual_ || wrapped || def_impl || melange_mode
      in
      let emn_not_filtrable emn =
        (* FIXME: ocamldep doesn't see Melange_wrapper for files that
                       have been `copy_files` *)
        String.equal "Melange_wrapper" (Module_name.to_string emn)
        || flag_open_present (Module_name.to_string emn) flags
      in
      if
      ...
      then Resolve.Memo.return libs
        ...
        let rm_libs =
          List.map libs ~f:(fun lib ->
              if lib_not_is_filterable lib then Resolve.Memo.return (Some lib)
              else
                ...
                if List.is_non_empty entry_module_names then
                  let fm =
                    List.map entry_module_names ~f:(fun entry_module_name ->
                        if emn_not_filtrable entry_module_name then
                          Resolve.Memo.return true
                        else
                          (* Easy case *)
                          let keep1 =
                            List.exists entry_module_names ~f:(fun emn ->
                                exists_in_odeps (Module_name.to_string emn))
                          in
                          if keep1 then Resolve.Memo.return true
                          else
                            (* Closure case *)
                            let rmlopt =
                              Resolve.Memo.map lib_top_module_mapm
                                ~f:(fun lib_top_module_map ->
                                  let lib_top_module_map =
                                    List.concat lib_top_module_map
                                    |> Module_name.Map.of_list
                                    |> Result.value
                  ...
                  in
                  let exists1 = Resolve.Memo.all fm in
                  Resolve.Memo.map exists1 ~f:(fun el ->
                      if List.exists el ~f:(fun x -> x) then Some lib else None)
                else Resolve.Memo.return (Some lib))
        in
        let+ libs =
          Resolve.Memo.all rm_libs
          |> Resolve.Memo.map ~f:(fun libs ->
                 List.filter_map libs ~f:(fun lb -> lb))
        in
        libs
