...
let copy_interface ~sctx ~dir ~obj_dir ~cm_kind m =
  ...
  Memo.when_
    ...
    (fun () ->
      let cmi_kind = Lib_mode.Cm_kind.cmi cm_kind in
      Super_context.add_rule ~from:"copy interface" sctx ~dir
...
let build_cm cctx ~force_write_cmi ~precompiled_cmi ~cm_kind (m : Module.t)
    ...
    let rec tostr acc (i : Command.Args.without_targets Command.Args.t) =
      ...
      let v =
        ...
        | S lst ->
          ...
          ^ (List.map lst ~f:(fun x -> tostr acc x) |> String.concat ~sep:",")
        | Concat _ -> "Concat"
  ...
  Super_context.add_rule ~from:"build_cm" sctx
    ...
    Action_builder.with_no_targets
      (Action_builder.bind module_deps ~f:(fun m ->
           let external_deps =
    ...
    Memo.when_ (ocaml.byte || ocaml.native) (fun () ->
        ...
        | Some src ->
          ...
          |> Memo.Option.iter ~f:(fun in_context ->
                 ...
                 in
                 action_with_targets
                 >>= Super_context.add_rule ~from:"build_module" sctx ~dir))
  in
...
let ocamlc_i ~(deps : Module_dep.t list Action_builder.t Ml_kind.Dict.t) cctx
  ...
  let modules = Compilation_context.modules cctx in
  Super_context.add_rule ~from:"ocamlc_i" sctx ~dir
...
let build_alias_module cctx group =
  ...
  let open Memo.O in
  let* () =
    Super_context.add_rule ~from:"build_alias_module" ~loc:Loc.none sctx ~dir
...
let build_root_module cctx root_module =
  ...
  let open Memo.O in
  let* () =
    Super_context.add_rule ~from:"build_root_module" ~loc:Loc.none sctx ~dir
...
let with_empty_intf ~sctx ~dir module_ =
  ...
  in
  let open Memo.O in
  let+ () = Super_context.add_rule ~from:"with_empty_intf" sctx ~dir rule in
